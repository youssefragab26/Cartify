  ...
  return (
    <div className="w-full">
      <div className="group relative w-full max-w-[220px] md:max-w-[260px] mx-auto bg-white m-2 md:m-2.5 rounded-3xl border border-gray-200/70 p-4 md:p-5 shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition duration-200 ease-out overflow-hidden">
        <div id="colorAndStorage" className="absolute top-3 left-3 flex items-center gap-2 z-10">
            <span className="px-2 py-0.5 text-[10px] md:text-xs rounded-full bg-gray-900 text-white shadow-sm">{productColor}</span>
            <span className="px-2 py-0.5 text-[10px] md:text-xs rounded-full bg-gray-100 text-gray-800 border border-gray-200 shadow-sm">{productStorage}</span>
        </div>
        ...
        <div className="text-center pt-3 md:pt-4 px-1">
          ...
          <p className="mt-1 text-sm md:text-base font-medium text-gray-600">{`${productPrice.toLocaleString()} EGP`}</p>
        </div>
